...
import { Stars, Cloud, ScrollControls, Scroll, useScroll, Html, Ring } from '@react-three/drei'; // Added Ring
import { motion, AnimatePresence } from 'framer-motion';
import { useNavigate } from 'react-router-dom';
import * as THREE from 'three';
import { useSound } from '../contexts/SoundContext';
import { Scan, Crosshair, Cpu, Gauge, Info, FastForward, Zap, Shield, Radar, Activity } from 'lucide-react'; // More icons
import { SCENE_SEQUENCE, PLANETARY_SYSTEM, NARRATIVE_TEXT } from '../config/landingConfig';
            ...
            {type === 'ringed' && (
                <group rotation={[1.5, 0, 0]}>
                    <Ring args={[size * 1.4, size * 2.2, 64]} >
                        <meshStandardMaterial color="#CDB482" opacity={0.8} transparent side={THREE.DoubleSide} />
                    </Ring>
                </group>
            )}
                ...
                >
                    <div className="flex flex-col items-start pl-4 border-l-2 border-cyan-400/50 relative">
                        ...
                        <p className="text-[10px] text-cyan-100/70 font-mono leading-relaxed max-w-[200px] uppercase tracking-wide">
                         ...
                         <p className="text-[9px] text-cyan-500/50 mt-1 font-mono">DIST: {(Math.random() * 100).toFixed(2)} AU</p>
                    </div>
                </div>
            </Html>
        </group>
    ...
    return (
        <group position={[0, -2, -100]} ref={earthRef}>
            ...
            {!performanceMode && (
             ...
             <pointLight position={[10, 5, 10]} intensity={2} color="#ffffff" distance={50} />
             <pointLight position={[-10, -5, 5]} intensity={0.5} color="#0044ff" distance={50} />
        </group>
    );
};
  ...
  useFrame((state, delta) => {
    if (starsRef.current) {
      if (isWarping) {
          ...
          starsRef.current.scale.z = THREE.MathUtils.lerp(starsRef.current.scale.z, 50, delta * 2);
...
const NebulaFog = ({ performanceMode }) => {
  ...
  return (
    <>
      ...
      <Cloud opacity={0.2} speed={0.2} width={60} depth={10} segments={20} position={[10, 0, -40]} color="#8b5cf6" />
      <Cloud opacity={0.1} speed={0.2} width={60} depth={10} segments={20} position={[-10, 5, -80]} color="#3b82f6" />
    </>
  );
};
...
const CameraRig = ({ setScrollOffset }) => {
  ...
  useFrame((state) => {
    ...
    // Bank based on turn
    state.camera.rotation.z = THREE.MathUtils.lerp(state.camera.rotation.z, -tangent.x * 0.5, 0.05);
...
const getActivePlanet = (scrollOffset) => {
    for (const planet of PLANETARY_SYSTEM) {
        if (scrollOffset >= planet.scrollRange[0] && scrollOffset < planet.scrollRange[1]) {
            return planet;
        }
    }
    return null;
...
const CockpitView = ({ scrollOffset }) => {
    ...
    const activePlanet = useMemo(() => getActivePlanet(scrollOffset), [scrollOffset]);

    return (
        <div className="absolute inset-0 z-40 pointer-events-none" style={{ opacity: Math.max(0, opacity) }}>
            ...
            <svg className="absolute inset-0 w-full h-full" preserveAspectRatio="none">
                 ...
                 {/* Top Bar */}
                 <path d="M0,0 L100,0 L100,50 L90,60 L10,60 L0,50 Z" fill="url(#frameGrad)" className="w-full" transform="scale(1, 1)" style={{ transformOrigin: 'top' }}/>
                 {/* Bottom Dashboard */}
                 <path d="M0,1000 L1000,1000 L1000,850 L800,800 L200,800 L0,850 Z" fill="url(#frameGrad)" />
            </svg>
            ...
            <AnimatePresence>
                {activePlanet && (
                    ...
                    >
                         ...
                         <div className="absolute -top-4 left-1/2 -translate-x-1/2 text-cyan-400 text-[10px] tracking-[0.3em] bg-black px-2">TARGET LOCKED</div>
                         
                         {/* Data Block */}
                         <div className="absolute top-[110%] left-1/2 -translate-x-1/2 bg-black/60 border-l-2 border-cyan-500 p-4 w-64 backdrop-blur-md">
                            <h4 className="text-xl font-bold text-cyan-100 uppercase tracking-widest mb-2 border-b border-white/10 pb-1">{activePlanet.name}</h4>
                            <div className="space-y-1 font-mono text-xs text-cyan-300">
                                ...
                                <div className="flex justify-between border-b border-cyan-500/20 pb-1">
                                    ...
                                    <span>{activePlanet.scanData?.gravity || "CALCULATING..."}</span>
                                </div>
                                <div className="flex justify-between border-b border-cyan-500/20 pb-1">
            ...
            <div className="absolute top-4 left-1/2 -translate-x-1/2 flex items-center gap-4 bg-black/80 px-6 py-2 rounded-b-xl border-b border-cyan-900 text-cyan-500 font-mono text-xs shadow-[0_0_20px_rgba(6,182,212,0.2)]">
                <span className="flex items-center gap-2"><Shield size={12} className="text-cyan-400" /> SHIELDS: 100%</span>
                <span className="w-[1px] h-4 bg-cyan-900"></span>
                <span className="flex items-center gap-2"><Zap size={12} className="text-yellow-400" /> POWER: NOMINAL</span>
                <span className="w-[1px] h-4 bg-cyan-900"></span>
                <span className="flex items-center gap-2 text-white"><Activity size={12} className="animate-pulse" /> CLOCK: {(scrollOffset * 100).toFixed(2)}</span>
            </div>
            ...
            <div className="absolute bottom-8 left-8 flex flex-col items-start gap-1 p-4 bg-black/40 backdrop-blur-sm border-l-4 border-cyan-500 rounded-r-lg skew-x-[-10deg]">
                <div className="flex items-end gap-2 text-cyan-400">
                    <span className="text-4xl font-bold font-header">{(Math.max(0, (scrollOffset + 0.1) * 29979)).toFixed(0)}</span>
                    <span className="text-sm font-mono mb-1">M/S</span>
                </div>
                <div className="w-48 h-2 bg-gray-800 rounded-full overflow-hidden">
                    <div className="h-full bg-gradient-to-r from-cyan-600 to-cyan-300" style={{ width: `${Math.min(100, scrollOffset * 150)}%` }}></div>
                </div>
                <span className="text-[10px] text-cyan-600 font-mono tracking-widest mt-1">THRUST OUTPUT</span>
            </div>
            ...
            <div className="absolute bottom-8 right-8 p-4 bg-black/40 backdrop-blur-sm border-r-4 border-cyan-500 rounded-l-lg skew-x-[10deg] flex flex-col items-end">
                ...
                <span className="text-[10px] text-cyan-600 font-mono tracking-widest mt-2">PROXIMITY SCAN</span>
            </div>

            {/* Center Reticle */}
            <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 opacity-60 pointer-events-none">
                 <Crosshair className="text-cyan-300 w-8 h-8 opacity-80" strokeWidth={1} />
                 {/* Reticle Decor */}
...
const CosmicLanding = () => {
  ...
  return (
    <div className="w-full h-screen bg-[#020408] text-white overflow-hidden relative selection:bg-cyan-500/30">
      ...
      <div className={`fixed inset-0 z-[100] bg-white pointer-events-none transition-opacity duration-1000 ease-in ${isWarping ? 'opacity-100' : 'opacity-0'}`} />
      ...
      <div 
        ...
        title={performanceMode ? "Enable High Quality" : "Enable Performance Mode"}
      >
          <Zap size={16} className={performanceMode ? "text-yellow-400" : "text-gray-500"} />
      </div>
      ...
      >
          <span className="text-[10px] font-mono tracking-widest text-white/50 group-hover:text-white">SKIP INTRO</span>
          <FastForward size={14} className="text-white/50 group-hover:text-cyan-400" />
      </div>

      {/* 3. The 3D World */}
      <Canvas camera={{ position: [0, 0, 5], fov: 75 }} gl={{ antialias: !performanceMode }} dpr={performanceMode ? 1 : [1, 2]}>
        <ScrollControls pages={SCENE_SEQUENCE.scrollPages} damping={SCENE_SEQUENCE.damping}>
          <CameraRig setScrollOffset={setScrollOffset} />
          <MovingStars scrollOffset={scrollOffset} performanceMode={performanceMode} isWarping={isWarping} />
      ...
      <div className="absolute inset-0 z-50 pointer-events-none">
          ...
          {/* Scene 1: Intro */}
          <div style={{ opacity: introOpacity }} className="fixed inset-0 flex flex-col items-center justify-center p-10 transition-opacity duration-100 ease-linear pointer-events-none">
           ...
           <div style={{ opacity: reflect1Opacity }} className="fixed inset-0 flex items-center justify-center text-center transition-opacity duration-100 ease-linear pointer-events-none">
           ...
           <div style={{ opacity: reflect2Opacity }} className="fixed inset-0 flex items-center justify-center text-center transition-opacity duration-100 ease-linear pointer-events-none">
               <p className="text-3xl md:text-5xl font-light text-cyan-100/90 font-serif italic mb-4 drop-shadow-md">
          ...
          {/* Scene 5: Finale */}
          <div style={{ opacity: finaleOpacity }} className="fixed inset-0 flex flex-col items-center justify-center transition-opacity duration-100 ease-linear pointer-events-none">
           ...
           {/* Scene 6: CTA */}
           <div style={{ opacity: ctaOpacity }} className="fixed inset-0 flex flex-col items-center justify-end pb-32 transition-opacity duration-100 ease-linear">
